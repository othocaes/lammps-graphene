...
# Usage: ./graph_avg_bond_length_afo_x_displacement.sh <LAMMPS dump file> <output file>

#	This program creates xmgrace-readable output from a LAMMPS dump file.
# Independent variable: x displacement of each atom
# Dependent variable: 	average bond length of that atom's bonds.
...
echo -n "Indexing values for atom 0/$num_atoms."
for atom_id in $(seq 1 $num_atoms); do
	echo -ne "\rIndexing values for atom $atom_id/$num_atoms."
	Z[$atom_id]=$(parse_coord x $dump_file $atom_id)
	U[$atom_id]=$(parse_avg_bond_length $dump_file $atom_id)
done
